        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {categories.map((category) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition cursor-pointer ${activeCategory === category ? 'bg-schoolBlue text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
            >
              {category}
            </button>
          ))}
          ...
          {filteredGallery.length > 0 ? (
            filteredGallery.map((item) => (
              <button
                ...
                className="group overflow-hidden rounded-[1.75rem] border border-slate-200 bg-slate-50 shadow-sm transition hover:-translate-y-1 hover:shadow-xl cursor-pointer"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 px-4 py-10">
          <div className="max-w-4xl w-full overflow-hidden rounded-[2rem] bg-white shadow-soft">
            <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-6 py-5">
              ...
              <button onClick={() => setSelected(null)} className="rounded-full border border-slate-200 px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 cursor-pointer">
                Close
              </button>
            </div>
